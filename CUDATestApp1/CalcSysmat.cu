...
namespace Reconstruction {
	...
	__global__ void calc_coeff_cbct_l(float* result, const int nd, const int center,
		...
		if (ix < nd && iy < nd) {
			...
			tan_angle = tanf(theta + phi);
			//intercept = offset_detector - rely + relx * tan_angle; //“–ŠYƒsƒNƒZƒ‹ã‚Å‚Ì‚™Ø•Ð
			intercept = sod_norm * tanf(phi) / (cosf(phi) * cosf(theta + phi)) - rely + relx * tan_angle; //“–ŠYƒsƒNƒZƒ‹ã‚Å‚Ì‚™Ø•Ð
